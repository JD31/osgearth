            ...
            // in viewport : go for decluttering
                ...
                if (attemptingPick) {
                    ...
                    if (box.contains(pickedPoint, pickingEpsilon)) {
                        ...
                        osg::NodePathList nodePathList = drawableParent->getParentalNodePaths();
                        osg::NodePath nodePath = drawableParent->getParentalNodePaths().at(0);

                        if (drawableParent && nodePathList.size() > 0) {

                            for (int i = 0; i < nodePath.size(); ++i) {

                                osg::ref_ptr<osg::Node> node = nodePath.at(i);

                                if (node != 0L && node->getOrCreateStateSet() != nullptr) {
                                    osg::Uniform* u =
